...
def generate_brd():
    ...
    brd_folder = os.path.join(project_root, "brd")
    brd_file = os.path.join(brd_folder, "Business_Requirements_Document.md")

    # Ensure BRD folder exists
...
"""
    ...
    if not os.path.exists(brd_file):
        with open(brd_file, "w", encoding="utf-8") as f:
            f.write(brd_content)
        print("BRD document created successfully at:", brd_file)
    else:
        print("BRD already exists. Delete it if you want to regenerate.")
